...
import 'package:thennal_music/API/clients.dart';
import 'package:thennal_music/DB/albums.db.dart';
import 'package:thennal_music/DB/playlists.db.dart';
...
import 'package:thennal_music/services/lyrics_manager.dart';
import 'package:thennal_music/services/proxy_manager.dart';
import 'package:thennal_music/services/settings_manager.dart';
import 'package:thennal_music/utilities/flutter_toast.dart';
import 'package:thennal_music/utilities/formatter.dart';
...
Future<List> getRecommendedSongs() async {
  try {
    // Always use recommendations from recently played songs if available
...
Future<void> getSimilarSong(String songYtId) async {
  try {
    // Add timeout for each operation to prevent blocking
    final song = await _yt.videos.get(songYtId).timeout(
      const Duration(seconds: 5),
      onTimeout: () {
        logger.log('Timeout getting video info for $songYtId', null, null);
        throw TimeoutException('Video fetch timeout');
      },
    );
    
    final relatedSongs = await _yt.videos.getRelatedVideos(song).timeout(
      const Duration(seconds: 5),
      onTimeout: () {
        logger.log('Timeout getting related videos for $songYtId', null, null);
        return null;
      },
    ) ?? [];
...
Future<List<dynamic>> getRelatedSongs(String songYtId, {int limit = 10}) async {
  try {
    final cacheKey = 'relatedSongs_$songYtId';
    ...
    final relatedSongs = <dynamic>[];
    for (var i = 0; i < relatedVideos.length && i < limit; i++) {
      relatedSongs.add(returnSongLayout(i, relatedVideos[i]));
    }

    if (relatedSongs.isNotEmpty) {
      await addOrUpdateData('cache', cacheKey, relatedSongs);
    }

    return relatedSongs;
  } catch (e, stackTrace) {
    logger.log('Error while fetching related songs:', e, stackTrace);
    return [];
  }
}
...
AudioStreamInfo selectAudioQuality(List<AudioStreamInfo> availableSources) {
  ...
  // 'high', 'veryhigh', and 'ultra' all use the highest quality
