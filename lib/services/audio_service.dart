...
import 'package:thennal_music/utilities/mediaitem.dart';
import 'package:rxdart/rxdart.dart';

class ThennalAudioHandler extends BaseAudioHandler {
  ThennalAudioHandler() {
    ...
    try {
      if (state == ProcessingState.completed) {
        if (!sleepTimerExpired && !_completionEventPending) {
          ...
          // Use Future.delayed instead of microtask to avoid blocking UI
  ...
  Future<void> _playNextRecommendedSong() async {
    ...
    try {
      ...
      // If recommendation is not already prefetched, fetch it
      if (nextRecommendedSong == null) {
        // Fetch with reasonable timeout (the getSimilarSong function itself has timeouts now)
        await _fetchRecommendedSong(baseSong).timeout(
          const Duration(seconds: 5),
          onTimeout: () {
            logger.log('Recommendation fetch timed out after 5 seconds', null, null);
          },
        );
      }
